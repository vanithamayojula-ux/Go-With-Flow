 ...
 * Provides soft multi-band toon ramps, painterly skybox gradients, vertex-driven wind sway,
 * warm-side/cool-shadow color splitting, directional rim lighting, soft gradient trails,
...
export const SkyboxShader = {
  ...
  fragmentShader: `
    ...
    // Pseudo-random noise for subtle painterly sky grain
    float noise(vec2 st) {
    ...
    void main() {
      ...
      // Painterly soft 3-band gradient sky with smooth transitions
      float band1 = smoothstep(0.0, 0.28, elevation);
      float band2 = smoothstep(0.20, 0.85, elevation);

      vec3 skyColor = mix(uSkyHorizon, uSkyMid, band1);
      skyColor = mix(skyColor, uSkyTop, band2);

      // Warm golden sun halo & atmospheric radial glow
      float sunDot = max(dot(dir, normalize(uSunPosition)), 0.0);
      float sunGlow = pow(sunDot, 10.0) * 0.40 + pow(sunDot, 48.0) * 0.65;
      skyColor += uSunColor * sunGlow;

      // Soft volumetric haze near horizon
      float haze = exp(-elevation * (10.0 - uHazeDensity * 5.0));
      vec3 hazeColor = mix(uSkyHorizon, uSunColor, 0.38);
      skyColor = mix(skyColor, hazeColor, haze * 0.72);

      // Subtle painterly atmospheric noise
...
export const TerrainShader = {
  ...
  fragmentShader: `
    ...
    uniform vec3 uSlopeWarmColor; // Sunlit slope tint (#F9E4B7)
    uniform vec3 uSlopeCoolColor; // Shaded slope tint (#4E8B69)
    ...
    void main() {
      ...
      // Organic tri-planar texture blending
      vec3 blending = abs(N);
      ...
      // Band 1: Ambient Shadow to Midtone
      float shadowToMid = smoothstep(-0.2, 0.15 + 0.1 * uCelRampHardness, NdotL);
      // Band 2: Midtone to Sunlit Highlight
      float midToSun = smoothstep(0.15 + 0.1 * uCelRampHardness, 0.65, NdotL);

      // Combine 3 tone bands (Shadow -> Midtone -> Sunlight)
      float celFactor = shadowToMid * 0.5 + midToSun * 0.5;

      // Warm-side / Cool-shadow color splitting
      vec3 warmSunSlope = mix(texColor, uSlopeWarmColor, 0.32);
      vec3 coolShadeSlope = mix(texColor, uSlopeCoolColor, 0.40);
      vec3 baseLit = mix(coolShadeSlope, warmSunSlope, celFactor);

      // Soft rim lighting on sunlit terrain ridges
      float rim = 1.0 - max(dot(V, N), 0.0);
      rim = smoothstep(0.48, 0.92, rim) * max(dot(N, L), 0.1);
      vec3 rimColor = uSunColor * (rim * uRimLightIntensity * 0.65);
...
export const FoliageShader = {
  vertexShader: `
    ...
    void main() {
      ...
      // Instance transform reconstruction
      ...
      // Vertex wind sway with macro-wave oscillation
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Two-sided soft cel lighting with leaf translucent backlight
      float NdotL = max(dot(N, L), 0.15);
      float backLight = max(dot(-N, L), 0.0) * 0.40; // Subsurface translucent glow

      // Soft 2-band cel step
      float celLight = smoothstep(0.1, 0.7, NdotL + backLight);

      // Sunlight gradient along blade height
      vec3 sunlitAlbedo = mix(texColor.rgb * 0.75, texColor.rgb * 1.3, vHeightFactor);

      // Soft golden rim lighting for foliage silhouettes
      float rim = 1.0 - max(dot(V, N), 0.0);
...
export const PostProcessShader = {
  ...
  fragmentShader: `
    ...
    // Pseudo-random generator for film grain / painterly texture noise
    float rand(vec2 co) {
    ...
    void main() {
      ...
      if (uBloom > 0.01) {
        ...
        float r = 2.5 * uBloom;
        bloomAccum += max(texture2D(tDiffuse, uv + vec2(-r, 0.0) * texel).rgb - 0.65, 0.0);
        bloomAccum += max(texture2D(tDiffuse, uv + vec2(r, 0.0) * texel).rgb - 0.65, 0.0);
        bloomAccum += max(texture2D(tDiffuse, uv + vec2(0.0, -r) * texel).rgb - 0.65, 0.0);
        bloomAccum += max(texture2D(tDiffuse, uv + vec2(0.0, r) * texel).rgb - 0.65, 0.0);
        sceneCol += bloomAccum * (0.35 * uBloom);
      }

      // Ghibli Color Grading: Nostalgic pastel look with warm highlight lift, lush greens, gentle cool shadow tint
      vec3 mapped = sceneCol * (2.4 * sceneCol + 0.12) / (sceneCol * (2.2 * sceneCol + 1.5) + 0.18);

      // Warm highlight lift & gentle midtone curve
      mapped.r = pow(mapped.r, 0.93);
      mapped.g = pow(mapped.g, 0.95);
      mapped.b = pow(mapped.b, 1.03); // slight cool shadow depth
      mapped += vec3(0.05, 0.04, 0.02) * uColorLift;

      // Subtle painterly film grain
